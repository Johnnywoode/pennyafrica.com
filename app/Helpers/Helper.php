...
class Helper
{
    ...
    public static function filterCountries(?array $dCodes, ?array $keys): array
    {
        $countries = self::countries();

        // If dCodes is null, include all countries
        if ($dCodes !== null) {
            $countries = array_filter($countries, fn($country) => in_array($country['d_code'], $dCodes));
        }

        // If keys is null, include all keys
        if ($keys !== null) {
            $countries = array_map(fn($country) => array_intersect_key($country, array_flip($keys)), $countries);
        }
